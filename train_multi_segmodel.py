...
def main():
	...
	train_ds, train_info = tfds.load('NyuDepthV2Labeled', split='train[:80%]',shuffle_files=True, as_supervised=True, with_info=True,)
	val_ds, val_info = tfds.load('NyuDepthV2Labeled', split='train[80%:]',shuffle_files=True, as_supervised=True, with_info=True,)
	test_ds, test_info = tfds.load('NyuDepthV2Labeled', split='test',shuffle_files=False, as_supervised=True, with_info=True,)
	...
	model = sm.Unet(BACKBONE,input_shape=(480, 640, 3),classes=2,activation='sigmoid')
	...
	model.save_weights(os.path.join(model_save_dir,'model_epoch10_diffscene.ckpt'))
	...
	for image, label in test_ds:
		for j in range(image.shape[0]):
			...
			Image.save_img(os.path.join(testing_save_path,str(i*batch_size+j).zfill(4)+'_image.png'), image[j])
			Image.save_img(os.path.join(testing_save_path,str(i*batch_size+j).zfill(4)+'_trueseg.png'), label[j])
			# print(label[j])
			Image.save_img(os.path.join(testing_save_path,str(i*batch_size+j).zfill(4)+'_preddeg.png'), create_mask(pred_label[j]))
	...
	for image, label in val_ds:
		for j in range(image.shape[0]):
			pred_label = model.predict(image)
			Image.save_img(os.path.join(validating_save_path,str(i*batch_size+j).zfill(4)+'_image.png'), image[j])
			Image.save_img(os.path.join(validating_save_path,str(i*batch_size+j).zfill(4)+'_trueseg.png'), label[j])
			Image.save_img(os.path.join(validating_save_path,str(i*batch_size+j).zfill(4)+'_preddeg.png'), create_mask(pred_label[j]))
