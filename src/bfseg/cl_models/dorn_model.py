...
class DornModel(BaseCLModel):
  ...
  def __init__(self, run, root_output_dir):
    super(DornModel, self).__init__(run=run, root_output_dir=root_output_dir)
    # Loss weights
  ...
  def forward_pass(self, training, x, y, mask):
    ...
    pred_y_depth = tf.expand_dims(pred_y_depth, -1)
    pred_y_depth = tf.cast(pred_y_depth, tf.float32)
...
def ignorant_dorn_loss(depth_label, pred_depth_probs): 
  """
  wrapper to mask all "NaN" values in depth
  """
  pred_depth_probs_ignorant = tf.where(tf.math.is_nan(depth_label),
...
def dorn_loss_function(prob, depth): 
  """ Loss Function from DORN paper.
