...
class NyuDepthV2LabeledDepth(tfds.core.GeneratorBasedBuilder):
  ...
  def _info(self) -> tfds.core.DatasetInfo:
    ...
    return tfds.core.DatasetInfo(
        ...
        features=tfds.features.FeaturesDict({
            ...
            'label': tfds.features.FeaturesDict({'seg': tfds.features.Tensor(shape=(480, 640), dtype=tf.uint8), 'distance': tfds.features.Tensor(shape=(480, 640), dtype=tf.float32)})
        }),
    ...
    with h5py.File(dataset_path, 'r') as f:
      ...
      for i in range(images.shape[-1]):
        ...
        if scene_type is None or scene_types[i] == scene_type:
          ...
          yield str(i).zfill(4), {
              'image': images[:, :, :, i],
              'label': {'seg': combine_label, 'distance': depth}
          }
