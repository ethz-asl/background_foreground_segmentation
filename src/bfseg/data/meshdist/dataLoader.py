...
class DataLoader:
  ...
  def __init__(self,
    ...
    """
      ...
      Args:
          workingDir: Path to dataset images <- Required
          inputSize: Shape of the input image <- Required
    ...
    # Load validation dataset (ARCHE, CLA)
    ...
    if validationFilter is not None:
      self.validationFiles, self.validationLabels = self.applyFilterToValidFileNames(
          self.validationFiles, self.validationLabels, validationFilter)
    # Filter training images
    if trainFilter is not None:
      self.filenames, self.labels, self.depths = self.applyFilterToTrainFileNames(
          self.filenames, self.labels, self.depths, trainFilter)
  ...
  def applyFilterToValidFileNames(self, filenames, labels, filter):
    ...
    for i, name in enumerate(filenames):
      ...
      if route in filter.keys():
        if filter[route]['timestamp']['lower_bound'] < timestamp < filter[
            route]['timestamp']['upper_bound']:
    ...
    if "max_count" in filter.keys():
      ...
      # Oonly take every nth image
      keep_nth_image = len(filteredFilenames) // max_count
      return filteredFilenames[::keep_nth_image][
          0:max_count], filteredLabels[::keep_nth_image][0:max_count]

    return filteredFilenames, filteredLabels

  def applyFilterToTrainFileNames(self,
                                  filenames,
                                  labels,
                                  depths=None,
                                  filter={}):
    ...
    for filterRoute in filter.keys():
      ...
      if not os.path.exists(info_file):
        info_file = os.path.join(self.workingDir,
                                 f"../{filterRoute}/{filterRoute}_info.txt")

      df = pandas.read_csv(info_file, header=None, sep=',|;', engine="python")
      # only select image numbers that match timestamp
      valid_timestamps = (
          filter[filterRoute]['timestamp']['lower_bound'] <=
          df[1]) & (df[1] <= filter[filterRoute]['timestamp']['upper_bound'])
  ...
  def train_preprocess(self, image, labels):
    ...
    return image, {'depth': depth, 'semseg': label, 'combined': label}
