    ...
    def __init__(self,
        ...
        if validationMode is not "all":
            if validationMode is "ARCHE":
                ...
                self.validationFiles = [file for file in self.validationFiles if "_159" in file]
                self.validationLabels = [file for file in self.validationLabels if "_159" in file]
            elif validationMode is "CLA":
                # Ugly, change filenames in the hive data loader
                # Files with timestamp starting with _158 are from CLA
                self.validationFiles = [file for file in self.validationFiles if "_158" in file]
                self.validationLabels = [file for file in self.validationLabels if "_158" in file]
            else:
                print("Validation MODE", validationMode, "is unknown. Going to use all validation data")
