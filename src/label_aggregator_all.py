...
def callback(original, labels, distance, labels_dense, labels_depth):
  ...
  combined_labels = np.where(np_labels_dense == 2, aggregate_labels, np_labels_dense)
  dense_labels = np_labels_dense
  depth_labels = np_labels_depth
  unaggregated_labels = np_labels
  sparse_depth_labels = np_distance
  ...
  return img_msg, aggregate_labels, resized_original, original.encoding, dense_labels, combined_labels, depth_labels, unaggregated_labels, sparse_depth_labels
...
def getCallbackForTopic(topicNumber, publisher, counters):
  ...
  def m_callback(*args):
    ...
    if not counters[topicNumber] % rospy.get_param('~label_frequency', 1) == 0:
      return
    msg, sparse_labels, img, img_enc, dense_labels, combined_labels, depth_labels, unaggregated_labels, sparse_depth_labels = callback(*args)
    ...
    if rospy.get_param('~store_labels', False):
      ...
      cv2.imwrite(
          os.path.join(
              ...
              '{}_cam{}_sparselabels.png'.format(msg.header.stamp, topicNumber)),
          sparse_labels.astype('uint8'))
      cv2.imwrite(
          os.path.join(
              rospy.get_param('~label_path'),
              '{}_cam{}_denselabels.png'.format(msg.header.stamp, topicNumber)),
          dense_labels.astype('uint8'))
      cv2.imwrite(
          os.path.join(
              rospy.get_param('~label_path'),
              '{}_cam{}_combinedlabels.png'.format(msg.header.stamp, topicNumber)),
          combined_labels.astype('uint8'))  
      cv2.imwrite(
          os.path.join(
              rospy.get_param('~label_path'),
              '{}_cam{}_depthlabels.png'.format(msg.header.stamp, topicNumber)),
          depth_labels.astype('uint8'))     
      cv2.imwrite(
          os.path.join(
              rospy.get_param('~label_path'),
              '{}_cam{}_unaggregatedlabels.png'.format(msg.header.stamp, topicNumber)),
          unaggregated_labels.astype('uint8'))   
      cv2.imwrite(
          os.path.join(
              rospy.get_param('~label_path'),
              '{}_cam{}_sparsedepthlabels.png'.format(msg.header.stamp, topicNumber)),
          sparse_depth_labels.astype('uint8'))  
...
def main():
  rospy.init_node('label_aggregator_all')
  # prepare directory
  ...
  for idx, topics in enumerate(rospy.get_param('~image_topics')):
    originalTopic, labelsTopic, distanceTopic, denseLabelsTopic, depthTopic = (topics[0], topics[1],
    ...
    depthSub = message_filters.Subscriber(depthTopic, Image)
