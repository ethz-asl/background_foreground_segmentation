...
class myMultiply(layers.Layer):
    def __init__(self, num_outputs, curr_block_name, **kwargs):
        super(myMultiply, self).__init__(name=curr_block_name + "_mul",
                                         **kwargs)
...
def build_model_with_adaptor(backbone, weights):
    ...
    for i in range(18):
        ...
        if pre_layer_name.endswith("pool"):
            ...
            new_x, old_x = add_lateral_connect(new_layer, old_layer, new_x,
                                               old_x, curr_block_name)
...
class Model:
    def __init__(self,
        ...
        self.new_model = build_model_with_adaptor(backbone, weights)
        # print(self.new_model.trainable_variables[-5])
    ...
    def train_step(self, train_x, train_y):
        ...
        with tf.GradientTape() as tape:
            ...
            # )
        grads = tape.gradient(output_loss, self.new_model.trainable_weights)
    ...
    def on_epoch_end(self, summary_writer, epoch, mode):
        ...
        with summary_writer.as_default():
            tf.summary.scalar('loss_ce',
                              self.loss_ce_tracker.result(),
                              step=epoch)
            tf.summary.scalar('accuracy', self.acc_metric.result(), step=epoch)
...
def main():
    ...
    log_dir = os.path.join('exp_PC', 'Progress', 'lateral', 'logs')
    model_save_dir = os.path.join('exp_PC', 'Progress', 'lateral',
                                  'saved_model')
    if step == "step1":
