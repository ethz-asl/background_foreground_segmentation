...
class myMultiply(layers.Layer):
    ...
    def build(self, input_shape):  # Create the state of the layer (weights)
        ...
        self.shape = tf.constant(
            [batch_size, input_shape[1], input_shape[2], 1], tf.int32)
...
class Progress(Base):
    ...
    def build_model(self):
        ...
        new_skips = [
            new_model.get_layer(name=i).output
            for i in new_skip_connection_layers
        ]
        old_skips = [
            old_model.get_layer(name=i).output
            for i in old_skip_connection_layers
        ]
        for i in range(18):
            if i == 0:
                new_x = new_input
        ...
        for i in range(18, len(new_model.layers)):
            ...
            if curr_layer_name.startswith(
                    "decoder") and curr_layer_name.endswith("conv"):
                ...
                new_x = new_layer(new_x)
                old_x = old_layer(old_x)
        self.new_model = keras.Model(inputs=[new_input, old_input],
                                     outputs=new_x)
