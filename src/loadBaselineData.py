...
def normalize_img(image, label):
  ...
  input_size = (480, 720)
  return cropImageToInputSize(image, input_size), cropImageToInputSize(
      label, input_size, method="nearest")


def cropImageToInputSize(image, size, method="bilinear"):
  ...
  cut_left = tf.cast((image_w - tf.cast(crop_width, tf.float64)) / 2., tf.int32)
...
def getDataSets(batch_size, inputShape=(720, 480)):
