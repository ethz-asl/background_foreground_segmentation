...
class EWC(Base):
    ...
    def load_dataset(self, train_dataset, train_scene, test_dataset, test_scene, data_dir, batch_size):
        """ Create 3 dataloaders for training, validation and testing """
        train_ds = self.load_data(train_dataset, data_dir, 'train', batch_size, train_scene)
        val_ds = self.load_data(train_dataset, data_dir, 'val', batch_size, train_scene)
        test_ds = self.load_data(test_dataset, data_dir, 'test', batch_size, test_scene)
        pretrain_ds = self.load_data(test_dataset, data_dir, 'train',
                                     batch_size, test_scene)
    ...
    def create_fisher_params(self, dataset):
        ...
        for i in range(len(fisher_param_names)):
            single_fisher_param_list = [
                tf.square(param[i]) for param in grads_list
            ]
    ...
    def run(self):
        ...
        train_ds, val_ds, test_ds, pretrain_ds = self.load_dataset(
            self.config.train_dataset, self.config.train_scene, self.config.test_dataset,
            self.config.test_scene, self.config.data_dir, self.config.batch_size)
