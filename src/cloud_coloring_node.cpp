...
float getRGB(uchar r, uchar g, uchar b) {
  union {
    uint32_t color;
    float color_float;
  } rgb;
  rgb.color = (static_cast<uint32_t>(r) << 16) |
              (static_cast<uint32_t>(g) << 8) | static_cast<uint32_t>(b);
  return rgb.color_float;
}

pcl::PointCloud<pcl::PointXYZRGB>
color_pc(const pcl::PointCloud<pcl::PointXYZ> &cloud,
  ...
  for (pcl::PointCloud<pcl::PointXYZ>::const_iterator it = cloud.begin();
    ...
    if (pixel.x >= 0 && pixel.x < camera_image.cols && pixel.y >= 0 &&
      ...
      new_point.rgb = getRGB(color[2], color[1], color[0]);
  ...
  for (uint i = 0; i < num_cameras; i++) {
    ...
    if (cam_infos[i] == nullptr) {
      ...
      if (image_frames.size() > 0) {
        out_cloud +=
            color_pc(in_cloud, images[i], cam_infos[i], image_frames[i]);
      } else {
        std::string frame = images[i]->header.frame_id;
        out_cloud += color_pc(in_cloud, images[i], cam_infos[i], frame);
      }
    }
  }
...
int main(int argc, char **argv) {
  ros::init(argc, argv, "cloud_coloring");
  ros::NodeHandle nh;
  ...
  ros::NodeHandle("~").getParam("output_topic", output_topic);
